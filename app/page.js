    ...
    try {
      ...
      if (current) {
        const [fileRes, memberRes, taskRes, noteRes] = await Promise.all([
          ...
          supabase.from("workspace_members").select("workspace_id,user_id,role,profiles(id,email,display_name)").eq("workspace_id", current.id),
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="workspace-picker">
          ...
          <select value={workspace?.id || ""} onChange={(e) => {
        ...
        {!workspace ? (
          ...
          <>
            ...
            {page === "Members" && <DataPage title="Members" text="People with access to this workspace.">
              <div className="list">{members.map(m => <div className="list-row" key={m.user_id}><Users size={18}/><div><strong>{m.profiles?.display_name || m.profiles?.email || m.user_id}</strong><span>{m.role}</span></div></div>)}</div>
            </DataPage>}
...
function SetupScreen() {
  return <div className="center-screen"><div className="setup-card"><Shield size={34}/><h1>Connect Supabase</h1><p>Add <code>NEXT_PUBLIC_SUPABASE_URL</code> and <code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> to Vercel Environment Variables, then redeploy.</p><p className="muted">Run the SQL in <code>supabase/schema.sql</code> first.</p></div></div>;
}
